...
const Analysis = () => {
  ...
  const [storageInfo, setStorageInfo] = useState({ used: 0, total: 1024, percentage: 0 }) // MB
  const [showSelectiveMode, setShowSelectiveMode] = useState(false)
  const [selectedRecords, setSelectedRecords] = useState(new Set())
  const [showSelectiveDeleteModal, setShowSelectiveDeleteModal] = useState(false)
    ...
    const loadData = async () => {
      try {
        ...
        const transformedData = progressData.map(item => {
          ...
          const facultiesInvolved = [...new Set((item.PROGRESS || []).map(p => p['Faculty name'] || p.facultyName))].join(', ')
          
          // Calculate the ACTUAL current state of this chapter-branch combination
          const lectureTypesArray = (() => {
            if (!item.PROGRESS || item.PROGRESS.length === 0) return ['REGULAR']
            
            const types = []
            
            // Check if ANY lecture is overshoot (if so, the whole thing is overshoot status)
            const hasOvershoot = item.PROGRESS.some(progress => 
            ...
            const hasSubstitute = item.PROGRESS.some(progress => 
              progress.lectureTypes && (
                Array.isArray(progress.lectureTypes) 
          ...
          return {
            ...
            'TOTALNOOF LECTURES': item['TOTALNOOF LECTURES'] || item.totalLectures || '0',
        ...
        if (transformedData.length === 0) {
          toast.info('No lecture progress data found. Faculty members need to submit some lectures first.')
        }
    ...
    try {
      ...
      const exportData = data.map(item => {
        ...
        const progressEntries = item.PROGRESS.map((progress, index) => ({
          ...
          'Faculty for This Entry': progress['Faculty name'] || progress.facultyName || 'Unknown',
          ...
          'Date': progress.DATE ? new Date(progress.DATE).toLocaleDateString() : '',
    ...
    dataArray.forEach(item => {
      ...
      if (item.PROGRESS && Array.isArray(item.PROGRESS)) {
        item.PROGRESS.forEach(progress => {
          ...
          const remarkLength = (progress.overshootRemark || '').length + (progress.substituteRemark || '').length
  ...
  const handleSelectiveDelete = async () => {
    try {
      ...
      const deletePromises = selectedUUIDs.map(uuid => {
        ...
        return lectureProgressService.delete(record.id || record.UUID)
      })
      
      await Promise.all(deletePromises)
      
      // Update local data
      const remainingData = data.filter(item => !selectedRecords.has(item.UUID))
      ...
      toast.success(`Successfully deleted ${selectedUUIDs.length} selected records!`)
  ...
  return (
    <>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              fontWeight: '600',
              transition: 'all 0.2s ease',
              boxShadow: data.length === 0 ? 'none' : '0 2px 4px rgba(16, 185, 129, 0.3)'
            }}
          >
            <Download size={18} />
            Export Excel
          ...
          <button
            ...
            style={{
              ...
              gap: '8px',
              padding: '12px 16px',
              backgroundColor: data.length === 0 ? '#d1d5db' : showSelectiveMode ? '#f59e0b' : '#3b82f6',
              color: 'white',
              border: 'none',
              borderRadius: '8px',
              cursor: data.length === 0 ? 'not-allowed' : 'pointer',
              ...
              fontWeight: '600',
              transition: 'all 0.2s ease',
              boxShadow: data.length === 0 ? 'none' : showSelectiveMode ? '0 2px 4px rgba(245, 158, 11, 0.3)' : '0 2px 4px rgba(59, 130, 246, 0.3)'
            }}
          >
            <input 
              type="checkbox" 
            ...
            />
            {showSelectiveMode ? 'Cancel Selection' : 'Select Records to Delete'}
          </button>
          ...
          <div style={{
            ...
            border: `2px solid ${storageInfo.percentage > 80 ? '#ef4444' : storageInfo.percentage > 60 ? '#f59e0b' : '#10b981'}`,
            borderRadius: '8px',
            minWidth: '200px'
          }}>
            {/* Circular Progress */}
            <div style={{
              ...
              <svg width="36" height="36" style={{ transform: 'rotate(-90deg)' }}>
                ...
                <circle
                  ...
                  cy="18"
                  r="16"
                  stroke={storageInfo.percentage > 80 ? '#ef4444' : storageInfo.percentage > 60 ? '#f59e0b' : '#10b981'}
                  strokeWidth="3"
                  fill="none"
                  strokeDasharray={`${2 * Math.PI * 16}`}
                  strokeDashoffset={`${2 * Math.PI * 16 * (1 - storageInfo.percentage / 100)}`}
              ...
              <div style={{
                ...
                fontSize: '10px',
                fontWeight: '600',
                color: storageInfo.percentage > 80 ? '#ef4444' : storageInfo.percentage > 60 ? '#f59e0b' : '#10b981'
              }}>
      ...
      <div className="content-padding" style={{ padding: '20px' }}>
        ...
        {/* Results count */}
        <div style={{ marginBottom: '15px', color: '#666', fontSize: 'clamp(12px, 2.5vw, 14px)' }}>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '20px 20px 0 20px', flexShrink: 0 }}>
              <h3 style={{ marginBottom: '20px', color: '#333', fontSize: 'clamp(16px, 3vw, 18px)' }}>
                Faculty Data
              </h3>
              ...
              {!isMobile && (
                <div
                  ...
                  style={{
                    display: 'grid',
                    gridTemplateColumns: showSelectiveMode ? '40px repeat(7, 1fr)' : 'repeat(7, 1fr)',
                    gap: '10px',
                    padding: '15px',
                    marginBottom: '15px',
                ...
                >
                  {showSelectiveMode && (
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                      <input
                        type="checkbox"
                        checked={selectedRecords.size === filteredAndSortedData.length && filteredAndSortedData.length > 0}
                  ...
                  <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>Chapter - Branch</div>
                  <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>Faculties</div>
                  <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>Subject</div>
                  <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>Branch</div>
                  <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>Chapter</div>
                  <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>Type</div>
                  <div style={{ fontSize: 'clamp(10px, 2vw, 14px)', textAlign: 'center' }}>
                    Progress
                  </div>
                </div>
              )}
              ...
              {loading ? (
                ...
                filteredAndSortedData.map((item) => {
                  ...
                  return (
                    <div
                      key={item.UUID}
                      className={`mobile-card ${selectedRow?.UUID === item.UUID ? 'selected' : ''}`}
                      onClick={() => setSelectedRow(item)}
                    >
                      {/* Card Header with Name and Progress */}
                      <div className="mobile-card-header">
                        <div>
                          <h4 className="mobile-card-title">{item['Faculty name']}</h4>
                          <p className="mobile-card-subtitle">{item.SUBJECT} | Faculties: {item['Faculty code']}</p>
                        </div>
                      ...
                      <div className="mobile-card-details">
                        <div className="mobile-card-detail">
                          ...
                          <span className="mobile-card-detail-value">{item['BRANCH NAME']}</span>
                        </div>
                        <div className="mobile-card-detail">
                          <span className="mobile-card-detail-label">Chapter</span>
                          <span className="mobile-card-detail-value">{item.CHAPTERNAME}</span>
                        </div>
                      </div>
                ...
                filteredAndSortedData.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {showSelectiveMode && (
                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                          <input
                            type="checkbox"
                            checked={selectedRecords.has(item.UUID)}
                            onChange={(e) => handleSelectRecord(item.UUID, e.target.checked)}
                            style={{ 
                              width: '16px', 
                              height: '16px',
                              cursor: 'pointer'
                      ...
                      <div style={{ fontSize: 'clamp(10px, 2vw, 14px)', fontWeight: '500' }}>
                        {item['Faculty name']}
                      </div>
                      <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>
                        {item['Faculty code']}
                      </div>
                      <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>{item.SUBJECT}</div>
                      <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>
                        {item['BRANCH NAME']}
                      </div>
                      <div style={{ fontSize: 'clamp(10px, 2vw, 14px)' }}>{item.CHAPTERNAME}</div>
                      <div
                        style={{
                          display: 'flex',
                          flexWrap: 'wrap',
                      ...
                      >
                        <div style={{ width: 'clamp(30px, 6vw, 50px)', height: 'clamp(30px, 6vw, 50px)' }}>
                          <CircularProgressbar
                            value={Math.min(progressPercentage, 100)}
                            text={`${currentLecture}/${totalLectures}`}
                            styles={buildStyles({
            ...
            >
              {selectedRow ? (
                <div style={{ position: 'relative' }}>
                  ...
                  >
                    <h4 style={{ margin: '0 0 10px 0', color: '#1976d2', fontSize: 'clamp(14px, 2.5vw, 16px)' }}>
                      {selectedRow['Faculty name']} ({selectedRow.SUBJECT})
                    </h4>
                    <p style={{ margin: '5px 0', fontSize: 'clamp(12px, 2vw, 14px)' }}>
                    ...
                    <div>
                      {[
                        ...
                        ...selectedRow.PROGRESS.map((progress, idx) => ({
                          ...
                          subtitle: (() => {
                            let subtitle = `${progress['Faculty name'] || progress.facultyName || 'Unknown'} • ${new Date(progress.DATE).toLocaleDateString('en-US', {
                              year: 'numeric',
                              month: 'short',
                              day: 'numeric',
                            })}`
                            ...
                            if (progress.overshootRemark && progress.overshootRemark.trim()) {
                              subtitle += ` 🔴 OVERSHOOT: ${progress.overshootRemark.trim()}`
                            }
                            
                            // Add substitute remark directly to this lecture entry
                            if (progress.substituteRemark && progress.substituteRemark.trim()) {
                        ...
                        <div key={index} className="mobile-timeline-item">
                          <div className={`mobile-timeline-dot ${item.type === 'end' ? 'end' : ''} ${index === array.length - 1 ? 'last' : ''}`} />
                          <div className="mobile-timeline-content">
                            <h5 className="mobile-timeline-title">{item.title}</h5>
                            <p className="mobile-timeline-subtitle">{item.subtitle}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    >
                      {[
                        ...
                        <React.Fragment key={index}>
                          ...
                          >
                            ...
                            <div
                              style={{
                                ...
                                borderRadius: '50%',
                                backgroundColor: item.type === 'end' ? '#333' : '#666',
                                border: '3px solid white',
                                boxShadow: `0 0 0 2px ${item.type === 'end' ? '#333' : '#666'}`,
                                marginTop: '8px',
                                zIndex: 2,
                                position: 'relative',
                              }}
                          ...
                          <div
                            style={{
                              paddingBottom: index < array.length - 1 ? '40px' : '0',
                              paddingTop: '8px',
                            }}
                          >
                            <div
                ...
                >
                  {isMobile ? 'Tap on any card to view progress timeline' : 'Click on any row to view progress timeline'}
                </div>
              )}
            </div>
          </div>
            ...
            >
              ...
              <div style={{
                ...
                <p style={{ 
                  ...
                  This will permanently delete the selected lecture progress data from the database.
                </p>
              </div>
              ...
              <div style={{ padding: '20px' }}>
                <div style={{
                  ...
                  <h4 style={{ margin: '0 0 12px 0', color: '#c2410c', fontSize: '14px' }}>
                  ...
                  <ul style={{ margin: '0', paddingLeft: '16px', color: '#9a3412' }}>
                    {Array.from(selectedRecords).map((uuid) => {
                      const record = data.find(item => item.UUID === uuid)
                      return (
                        <li key={uuid} style={{ fontSize: '13px', marginBottom: '4px' }}>
                          {record['Faculty name']} - {record.SUBJECT} (Lecture: {record.LECTURENUMBER})
                        </li>
                      )
                    })}
                  </ul>
                ...
                <div style={{
                  ...
                  <p style={{ margin: '0', color: '#075985', fontSize: '13px' }}>
                    Export data to Excel first using the "Export Excel" button to keep a backup before deleting.
                  </p>
                </div>
                ...
                <p style={{ 
                  ...
                  ⚠️ This action cannot be undone and will free up Firebase storage space.
                </p>
              </div>
